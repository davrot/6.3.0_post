...
export default async function (webRouter, privateApiRouter, publicApiRouter) {
    ...
    expressify(async function (req, res, next) {
      ...
      const ltAvailable =
        ...
        !!(
          ...
          process.env.LANGUAGETOOL_URL // [VI] accept the legacy LANGUAGETOOL_URL env (deploy speaks that name)
        )
  ...
  webRouter.use(async function (req, res, next) {
    ...
    res.locals.ExposedSettings = {
      ...
      // 2026-09: the Dropbox project-mirror feature is only usable when its
      // OAuth keys are configured (DropboxRouter reads exactly these two
      // envs). Expose the gate so the settings widget can hide itself
      // instead of 500-ing on /user/dropbox/status for unconfigured boxes.
      dropboxEnabled: !!(process.env.DROPBOX_APP_KEY && process.env.DROPBOX_APP_SECRET),
      ...
      webdavEnabled: !!Settings.webdav, // [III] Nextcloud/WebDAV project mirroring gate
      enablePandocConversions: Settings.enablePandocConversions,
      mixpanelLabsToken:
        Settings.labs?.enable && Settings.analytics?.mixpanel?.labsToken,
      // overleaf-lab: BYO LLM user settings gate (Account Settings section)
      ...
      // overleaf-lab (grammar port): single JSON blob consumed by getMeta('ol-grammarSettings') in the
