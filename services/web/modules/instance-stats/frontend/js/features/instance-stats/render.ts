...
async function waitForPlotly(timeoutMs = 15000): Promise<void> {
  if (typeof window === 'undefined') return
  if ((window as Window & { Plotly?: unknown }).Plotly) return
...
export async function renderChart(stat: StatConfig, points: SeriesPoint[]) {
  // The page renders as soon as the app bundle runs, but the (deferred)
  // Plotly script may still be loading — poll briefly instead of throwing
  // "Plotly is not available on window" and silently skipping the charts
  // (2026-09 owner report: axes rendered, plots never appeared).
  await waitForPlotly()
  if (!window.Plotly) {
