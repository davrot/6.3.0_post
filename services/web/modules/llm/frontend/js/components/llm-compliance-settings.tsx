...
import Notification from '@/shared/components/notification' // 6.3.0 design system: legacy Notification (OLNotification alias)
        ...
        runAsync(
            (async () => {
                try {
                    ...
                    // runAsync(promise) rejects → handled below with the message
                    throw new Error(err?.data?.message || err?.message || t('save_failed', 'Saving failed'))
                }
            })(),
        )
            .then(() => {
            ...
            .catch((err: any) => {
                setSaving(false)
                setSaveError(typeof err?.message === 'string' && err.message ? err.message : t('save_failed', 'Saving failed'))
            })
    }
        ...
        <div className="llm-compliance-settings">
            ...
            {saved && (
                <div className="ol-llm-admin-settings__mt-md">
                    <Notification type="success" content={t('compliance_saved', 'Your rubrics are saved and now personal.')} />
                </div>
            )}
            {saveError && (
                <div className="ol-llm-admin-settings__mt-md">
                    <Notification type="error" content={saveError} />
                </div>
            )}
        </div>
    )
