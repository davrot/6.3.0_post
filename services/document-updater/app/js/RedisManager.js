...
const RedisManager = {
  ...
  async recordProjectNotificationTimestamp(projectId, timestamp, userId) {
    // [IV] 1h TTL: these keys are one-shot batch markers consumed by the
    // enqueue cron (or, for projects without collaborators, never consumed).
    // Without a TTL, every edit in a solo project would leak a key forever.
    ...
    // [IV] Store who made this change next to the timestamp (same NX
